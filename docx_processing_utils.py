...
def has_special_formatting(run):
    try:
        if run.font.subscript or \
           run.font.superscript or \
           run.font.bold or \
           run.font.italic: 
            return True
...
def split_text_by_proportions(text_to_split, original_runs, original_full_text, placeholder_map_for_segment):
    ...
    is_placeholder_for_full_segment = text_to_split in placeholder_map_for_segment and \
                                      placeholder_map_for_segment[text_to_split] == original_full_text
    ...
    for i, run_text in enumerate(original_run_texts):
        ...
        estimated_len = max(0, int(len(remaining_text_to_split) * proportion))
...
def _get_run_formatting(run):
    fmt = {
        ...
        'size': None, 'name': None, 'color_rgb': None,
    }
    try:
        font = run.font
        fmt['bold'] = font.bold
...
def collect_texts_and_images(doc, img_dir_base, entities, docx_path_for_docx2txt):
    texts_collected = [] 
    image_positions_map = {}
    ...
    try:
        docx2txt.process(docx_path_for_docx2txt, img_dir_base)
    except Exception as e:
        print(f"Warning: docx2txt processing failed: {e}. Image extraction might be incomplete.")

    extracted_image_files = []
    if os.path.exists(img_dir_base):
        extracted_image_files = sorted([
            os.path.join(img_dir_base, f) for f in os.listdir(img_dir_base)
            if f.lower().startswith('image') and f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff'))
        ], key=lambda x: int(re.search(r'image(\d+)', x, re.IGNORECASE).group(1)) if re.search(r'image(\d+)', x, re.IGNORECASE) else -1)
    
    current_image_idx = 0

    def process_paragraph_runs(para_obj, para_type_prefix, unique_para_id):
        nonlocal current_image_idx
        current_standard_group = []
        current_math_group = []
        run_list_for_para = list(para_obj.runs)

        for run_idx_in_para, run in enumerate(run_list_for_para):
            if is_equation_run(run):
                if current_standard_group:
                    ...
                    if group_text:
                        ...
                        texts_collected.append(("body", f"{para_type_prefix}_{unique_para_id}_std_{run_idx_in_para-len(current_standard_group)}", para_obj, group_text, mod_text, p_map, needs_translation(mod_text), mws, list(current_standard_group), fmt_info))
                    current_standard_group = []
                if current_math_group:
                    ...
                    if group_text.strip():
                        ...
                        texts_collected.append(("math_text", f"{para_type_prefix}_{unique_para_id}_math_{run_idx_in_para-len(current_math_group)}", para_obj, group_text, mod_text, p_map, needs_translation(mod_text), mws, list(current_math_group), fmt_info))
                    current_math_group = []
                
                equation_text = run.text
                texts_collected.append(("omath_object", f"{para_type_prefix}_{unique_para_id}_eq_{run_idx_in_para}", para_obj, equation_text, equation_text, {}, False, [], [run], [_get_run_formatting(run)]))
                continue
            ...
            if run._element.xpath('.//w:drawing'):
                ...
                if current_image_idx < len(extracted_image_files):
                    img_path = extracted_image_files[current_image_idx]
                    image_key = (para_type_prefix, unique_para_id, run_idx_in_para)
                    image_positions_map[image_key] = img_path
                    texts_collected.append(("image", image_key, para_obj, img_path, img_path, {}, False, [], [run], []))
                    current_image_idx += 1
                else:
                    texts_collected.append(("drawing_element", f"{para_type_prefix}_{unique_para_id}_drawing_{run_idx_in_para}", para_obj, "[[UNRESOLVED DRAWING]]", "[[UNRESOLVED DRAWING]]", {}, False, [], [run], []))
                continue
        ...
        if current_standard_group:
            ...
            if group_text:
                mod_text, p_map, mws = replace_with_placeholders(group_text, entities)
                fmt_info = [_get_run_formatting(r) for r in current_standard_group]
                texts_collected.append(("body", f"{para_type_prefix}_{unique_para_id}_std_end", para_obj, group_text, mod_text, p_map, needs_translation(mod_text), mws, list(current_standard_group), fmt_info))
        if current_math_group:
            group_text = "".join(r.text for r in current_math_group)
            if group_text.strip():
                mod_text, p_map, mws = replace_with_placeholders(group_text, entities)
                fmt_info = [_get_run_formatting(r) for r in current_math_group]
                texts_collected.append(("math_text", f"{para_type_prefix}_{unique_para_id}_math_end", para_obj, group_text, mod_text, p_map, needs_translation(mod_text), mws, list(current_math_group), fmt_info))
    ...
    if current_image_idx < len(extracted_image_files):
        remaining_image_files = extracted_image_files[current_image_idx:]

    return texts_collected, remaining_image_files, image_positions_map

def apply_formatting(target_runs, text_parts, formatting_info_list):
    num_runs_to_format = len(target_runs)

    for r_idx, r in enumerate(target_runs):
        if not is_equation_run(r) and not r._element.xpath('.//w:drawing'):
            r.text = ""

    for i in range(num_runs_to_format):
        run = target_runs[i]

        if is_equation_run(run) or run._element.xpath('.//w:drawing'):
            continue
        ...
        if i < len(formatting_info_list):
            ...
            try:
                ...
                if fmt.get('size') is not None and fmt['size'] is not None:
                    ...
                    if isinstance(current_size, Pt):
                         font.size = current_size
                    elif isinstance(current_size, (int, float)) and current_size > 0 :
